    ...
    <div class="space-y-6 sm:space-y-8">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                <a href="{{ route('notas.exportar.csv', request()->all()) }}" class="flex-1 md:flex-none px-4 sm:px-6 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition text-center">
                    Exportar CSV
                </a>
                <a href="{{ route('notas.create') }}" class="flex-1 md:flex-none px-4 sm:px-6 py-2 bg-primary-500 text-white rounded-xl text-sm font-bold hover:bg-primary-600 transition shadow-lg shadow-primary-200 text-center">
                    + Nova Nota
                </a>
            </div>
        </div>
        ...
        <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100">
            <form action="{{ route('notas.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 lg:gap-3 items-end">
                ...
                <div class="space-y-1 lg:col-span-3">
                    ...
                    <input type="text" name="busca" value="{{ request('busca') }}" placeholder="Empresa o CNPJ..."
                           class="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 outline-none transition">
                </div>
                <div class="space-y-1 md:col-span-2 lg:col-span-4 min-w-0">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2">
                        <input type="date" name="data_inicio" value="{{ request('data_inicio') }}" class="w-full min-w-0 px-3 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500 transition">
                        <span class="hidden sm:block text-gray-300 text-center">-</span>
                        <input type="date" name="data_fim" value="{{ request('data_fim') }}" class="w-full min-w-0 px-3 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary-500 transition">
                    </div>
                </div>
                <div class="flex items-end gap-2 md:col-span-2 lg:col-span-3">
                    <button type="submit" class="flex-1 px-4 py-2 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-gray-800 transition shadow-sm">Filtrar</button>
                    <a href="{{ route('notas.index') }}" class="px-4 py-2 bg-gray-100 text-gray-500 rounded-xl text-sm font-medium hover:bg-gray-200 transition text-center">Limpar</a>
                </div>
            </form>
        </div>
        ...
        <div class="md:hidden space-y-3">
            @forelse($notas as $nota)
                <a href="{{ route('notas.show', $nota) }}" class="block bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                    <div class="flex items-start gap-3">
                        <div class="h-10 w-10 bg-gray-100 rounded-xl flex items-center justify-center text-gray-500 flex-shrink-0">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="text-sm font-bold text-gray-900 truncate">{{ $nota->empresa_emissora ?? 'Pendente...' }}</p>
                                    <p class="text-xs text-gray-500">{{ $nota->data_emissao?->format('d/m/Y') ?? $nota->created_at->format('d/m/Y H:i') }}</p>
                                </div>
                                <p class="text-sm font-bold text-gray-900 flex-shrink-0">R$ {{ number_format($nota->valor_total, 2, ',', '.') }}</p>
                            </div>
        ...
        @if($notas->hasPages())
            <div class="md:hidden px-4 py-3 bg-white rounded-2xl shadow-sm border border-gray-100">
                {{ $notas->links() }}
            </div>
        @endif
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($notas as $nota)
                            <tr class="hover:bg-gray-50 transition group cursor-pointer" onclick="window.location='{{ route('notas.show', $nota) }}'">
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('notas.show', $nota) }}" onclick="event.stopPropagation()" class="p-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-primary-500 hover:text-white transition">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </a>
                                        <a href="{{ route('notas.edit', $nota) }}" onclick="event.stopPropagation()" class="p-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-blue-500 hover:text-white transition">
